            ...
            <form action="{{ route('admin.partners.update', $user->id) }}" method="POST">
                    ...
                    <div class="bg-slate-900/60 backdrop-blur-xl border border-white/20 rounded-3xl p-8 shadow-2xl">
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <input type="text" name="name" value="{{ old('name', $user->name) }}" class="w-full bg-slate-800/80 border border-white/10 rounded-xl text-white focus:ring-2 focus:ring-purple-500 transition h-12" required>
                            </div>
                            
                            <div>
                                <label class="block text-xs font-bold text-purple-300 uppercase mb-2">Email Address</label>
                                <input type="email" name="email" value="{{ old('email', $user->email) }}" class="w-full bg-slate-800/80 border border-white/10 rounded-xl text-white focus:ring-2 focus:ring-purple-500 transition h-12" required>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select name="company_type" class="w-full bg-slate-800/80 border border-white/10 rounded-xl text-white focus:ring-2 focus:ring-purple-500 transition h-12">
                                        <option value="Placement Agency" {{ (old('company_type', $profile->company_type ?? '') == 'Placement Agency') ? 'selected' : '' }}>Placement Agency</option>
                                        <option value="Freelancer" {{ (old('company_type', $profile->company_type ?? '') == 'Freelancer') ? 'selected' : '' }}>Freelancer</option>
                                        <option value="Recruiter" {{ (old('company_type', $profile->company_type ?? '') == 'Recruiter') ? 'selected' : '' }}>Recruiter</option>
                                    </select>
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-purple-300 uppercase mb-2">Est. Year</label>
                                    <input type="text" name="establishment_year" value="{{ old('establishment_year', $profile->establishment_year ?? '') }}" class="w-full bg-slate-800/80 border border-white/10 rounded-xl text-white focus:ring-2 focus:ring-purple-500 transition h-12">
                                </div>
                            </div>

                            <div>
                                <label class="block text-xs font-bold text-purple-300 uppercase mb-2">Bio / Description</label>
                                <textarea name="bio" rows="3" class="w-full bg-slate-800/80 border border-white/10 rounded-xl text-white focus:ring-2 focus:ring-purple-500 transition">{{ old('bio', $profile->bio ?? '') }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-slate-900/60 backdrop-blur-xl border border-white/20 rounded-3xl p-8 shadow-2xl">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="url" name="website" value="{{ old('website', $profile->website ?? '') }}" placeholder="https://" class="w-full bg-slate-800/50 border border-white/10 rounded-xl text-white h-10">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-slate-400 uppercase mb-1">LinkedIn</label>
                                <input type="url" name="linkedin_url" value="{{ old('linkedin_url', $profile->linkedin_url ?? '') }}" class="w-full bg-slate-800/50 border border-white/10 rounded-xl text-white h-10">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-slate-400 uppercase mb-1">Location / Address</label>
                                <textarea name="address" rows="2" class="w-full bg-slate-800/50 border border-white/10 rounded-xl text-white">{{ old('address', $profile->address ?? '') }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-slate-900/60 backdrop-blur-xl border border-white/20 rounded-3xl p-8 shadow-2xl lg:col-span-2">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                                ...
                                <input type="text" name="beneficiary_name" value="{{ old('beneficiary_name', $profile->beneficiary_name ?? '') }}" class="w-full bg-slate-800/80 border border-white/10 rounded-xl text-white h-12">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-emerald-300 uppercase mb-2">Account Number</label>
                                <input type="text" name="account_number" value="{{ old('account_number', $profile->account_number ?? '') }}" class="w-full bg-slate-800/80 border border-white/10 rounded-xl text-white h-12">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-emerald-300 uppercase mb-2">IFSC Code</label>
                                <input type="text" name="ifsc_code" value="{{ old('ifsc_code', $profile->ifsc_code ?? '') }}" class="w-full bg-slate-800/80 border border-white/10 rounded-xl text-white h-12">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-emerald-300 uppercase mb-2">PAN Number</label>
                                <input type="text" name="pan_number" value="{{ old('pan_number', $profile->pan_number ?? '') }}" class="w-full bg-slate-800/80 border border-white/10 rounded-xl text-white h-12">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-emerald-300 uppercase mb-2">GST Number</label>
                                <input type="text" name="gst_number" value="{{ old('gst_number', $profile->gst_number ?? '') }}" class="w-full bg-slate-800/80 border border-white/10 rounded-xl text-white h-12">
                            </div>
                        </div>
                    </div>
                ...
                <div class="flex justify-end gap-4 border-t border-white/10 pt-8 mt-8 pb-12">
                    ...
                    <button type="submit" class="px-10 py-3 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white rounded-xl font-bold shadow-lg shadow-purple-600/30 transition transform hover:-translate-y-1 flex items-center gap-2">
                        <i class="fa-solid fa-floppy-disk"></i> Update Partner
                    </button>
                </div>
            </form>
        </div>

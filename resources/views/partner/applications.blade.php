            ...
            <div class="flex items-center gap-3 px-1 min-w-max">
                @foreach($tabs as $t)
                    ...
                    <a href="{{ route('partner.applications', $linkParams) }}"
                       class="inline-flex items-center gap-3 pl-4 pr-1.5 py-1.5 rounded-full text-xs font-bold border whitespace-nowrap transition
                        ...
                        <span class="inline-flex items-center justify-center min-w-[1.5rem] h-6 px-2 rounded-full text-[11px] font-extrabold leading-none border {{ $isActive ? 'bg-white/30 text-white border-white/30' : 'bg-white/15 text-white border-white/20' }}">{{ $t['count'] }}</span>
                    </a>
                @endforeach
            </div>
        </div>
